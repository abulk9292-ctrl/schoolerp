...
class Salary(models.Model):
    ...
    extra_deduction = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    ...
    def get_previous_due_amount(self):
        ...
        previous_records = Salary.objects.filter(employee=self.employee).exclude(pk=self.pk)
        ...
        for record in previous_records:
            ...
            if record.year < self.year or (record.year == self.year and record_month_no < current_month_no):
    ...
    def calculate_payment_status(self):
        ...
        if not self.payable_amount:
            self.payable_amount = self.net_salary + self.previous_due

        if self.paid_amount >= self.payable_amount:
            self.due_amount = Decimal('0.00')
            self.advance_amount = self.paid_amount - self.payable_amount
            self.status = 'Advance' if self.advance_amount > 0 else 'Paid'
    ...
    def calculate_salary(self):
        ...
        self.month_days = calendar.monthrange(self.year, month_number)[1]

        self.per_day_salary = (
            self.basic_salary / Decimal(self.month_days)
            if self.basic_salary else Decimal('0.00')
        )
        ...
        # absent 1  = no deduction, no bonus
        # absent >1 = absent - 1 days deduction
        if self.absent_days == 0:
            self.payable_absent_days = 0
            self.bonus = self.per_day_salary
        elif self.absent_days == 1:
            self.payable_absent_days = 0
            self.bonus = Decimal('0.00')
        else:
            self.payable_absent_days = self.absent_days - 1
            self.bonus = Decimal('0.00')

        self.attendance_deduction = self.per_day_salary * Decimal(self.payable_absent_days)
